...
def run_epoch_f(args, mode, model, criterion, optimizer,
    ...
    for step_idx, (X_ids, X_types, X_masks, Y) in tqdm(enumerate(data_loader),
        ...
        if mode == 'Train':
            ...
            pred_score = model(X_ids, X_types, X_masks)
            # Compute loss
        ...
        # Book-keeping
        current_batch_size = X_ids.size()[0]
...
def main():
    ...
    if args.task == 'Train':
        ...
        train_dataset = model_module.NEWS_dataset(train_dataset)
        dev_dataset = model_module.NEWS_dataset(dev_dataset)
        ...
        print(f'{datetime.datetime.now()}: Done')
        # Instantiate dataclasses
        test_dataset = model_module.NEWS_dataset(test_dataset)
        # Instantiate dataloaders
    ...
    # Instantiate model
    model = model_module.NEWS_model(args)
